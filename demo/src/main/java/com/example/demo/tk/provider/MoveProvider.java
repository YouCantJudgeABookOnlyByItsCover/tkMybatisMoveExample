package com.example.demo.tk.provider;
...
public class MoveProvider extends MapperTemplate {
    ...
    private String getSeqColumnName(Class<?> entityClass) {
        ...
        List<EntityColumn> seqColumns = columnList.stream().filter(column -> column.getEntityField().isAnnotationPresent(Seq.class)).collect(
                Collectors.toList());
    ...
    public String move(MappedStatement ms) {
        ...
        sql.append("update ").append(tableName(entityClass))
                .append(" <choose>")
                .append(" <when test=\"direction == @com.example.demo.tk.constantValues.MoveDirection@DownBefore\">")
                .append(" set " + seqColumnName + " = " + seqColumnName + " - 1 where " + seqColumnName + " &gt; #{movingSeq} and " + seqColumnName + " &lt; #{targetSeq}")
                .append(" </when>")
                .append(" <when test=\"direction == @com.example.demo.tk.constantValues.MoveDirection@DownAfter\">")
                .append(" set " + seqColumnName + " = " + seqColumnName + " - 1 where " + seqColumnName + " &gt; #{movingSeq} and " + seqColumnName + " &lt;= #{targetSeq}")
                .append(" </when>")
                .append(" <when test=\"direction == @com.example.demo.tk.constantValues.MoveDirection@UpBefore\">")
                .append(" set " + seqColumnName + " = " + seqColumnName + " + 1 where " + seqColumnName + " &gt;= #{targetSeq} and " + seqColumnName + " &lt; #{movingSeq}")
                .append(" </when>")
                .append(" <when test=\"direction == @com.example.demo.tk.constantValues.MoveDirection@UpAfter\">")
                .append(" set " + seqColumnName + " = " + seqColumnName + " + 1 where " + seqColumnName + " &gt; #{targetSeq} and " + seqColumnName + " &lt; #{movingSeq}")
                .append(" </when>")
                .append(" </choose>");
        //追加其他 where 条件
